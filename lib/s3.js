...
module.exports = function(options) {
  return function(req, res, next) {
    ...
    // If the request includes a if-none-match header, make a head request
    // to S3 to find out if the file has changed.
    // if (req.headers['if-none-match']) {
    //   s3.
    // }
    ...
    var readStream = s3Request.createReadStream()
      .on('error', function(err) {
        ...
        if (err.code === 'PreconditionFailed' && s3Options.IfNoneMatch) {
